...
<div class="row column">
    ...
    foreach($formCategoryData as $cat){
        $category = ($cat->getSelect_cat()) ? $cat : '';
        $parent_cat = $category ? $category->getId_Parent() : '';
    }
    if($formCategoryData){
    ?>
    ...
    <?php
        ...
        ?>
        <form action="<?= FO_URL ?>new/category/post"
              method="post">
            <div class="row">
                <div class="medium-4 columns">
                    <label for="nom">Nom
                        <input type="text" placeholder="Nom de la catégorie" id="name" name="name" value="">
                    </label>
                </div>
            </div>
            <div class="row">
                <div class="medium-4 columns">
                    <label for="icon">Icône
                        <input type="text" placeholder="Icône" id="icon" name="icon" value="">
                    </label>
                </div>
            </div>
            <div class="row">
